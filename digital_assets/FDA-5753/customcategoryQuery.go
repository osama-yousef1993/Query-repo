...
type CustomCategoryQuery interface {
	...
	FetchDataByTableName(ctx context.Context, query string, customCategoryFields datastruct.CustomCategoryRequest) ([]byte, error)
	...
	// GetCustomCategoriesDataFS retrieves all CustomCategory records from the FS to use it for rebuild the data.
...
func (c *customCategoryQuery) FetchDataByTableName(ctx context.Context, query string, customCategoryFields datastruct.CustomCategoryRequest) ([]byte, error) {
...
func (c *customCategoryQuery) InsertCustomCategories(ctx context.Context, data *datastruct.CustomCategory) error {
	...
	valueArgs := make([]interface{}, 0, tableFields)
	i := 0

	valString := fmt.Sprintf("($%d,$%d,$%d,$%d,$%d,$%d,$%d,$%d)", i*tableFields+1, i*tableFields+2, i*tableFields+3, i*tableFields+4, i*tableFields+5, i*tableFields+6, i*tableFields+7, i*tableFields+8)
	valueString = append(valueString, valString)

	valueArgs = append(valueArgs, data.CategoryName)
	valueArgs = append(valueArgs, data.CategorySlug)
	valueArgs = append(valueArgs, data.CategoryPath)
	valueArgs = append(valueArgs, data.CategoryType)
	valueArgs = append(valueArgs, data.IsHighlighted)
	valueArgs = append(valueArgs, !data.InActive)
	valueArgs = append(valueArgs, data.CategoryFields)
...
func (c *customCategoryQuery) GetCustomCategories(ctx context.Context, customCategoryType string) ([]datastruct.CustomCategory, error) {
	...
	for queryResult.Next() {
		...
		err := queryResult.Scan(&customCategory.CategoryName, &customCategory.CategorySlug, &customCategory.CategoryPath, &customCategory.CategoryType, &customCategory.IsHighlighted, &customCategory.InActive, &customCategory.CategoryFields, &customCategory.LastUpdated)
		if err != nil {
...
func (c *customCategoryQuery) GetCustomCategoriesDataFS(ctx context.Context) ([]datastruct.CustomCategoryRequest, error) {
	...
	dbSnap := fs.Collection(datastruct.CustomCategoryTable).Where("is_active", "==", true).Documents(ctx)
