...
func (c *customCategoryService) BuildCustomCategoriesDataFS(ctx context.Context) (map[string]store.CategoriesData, error) {
	...
	// categoryMap This is map will contains custom categories data with all nessacery fields to be added to our Categories Fundamentals table
	...
	// Get All custom categories that mark as FT categories to build it with Categories Fundamentals
	...
	// Map custom categories to CategoriesData struct so we can use it with Categories Fundamentals build
	for _, category := range categories {
		CategoryData := store.CategoriesData{ID: category.CategorySlug, Name: category.CategoryName, Inactive: category.InActive, IsHighlighted: category.IsHighlighted}
		markets, err := GetMarketDataFromCustomCategory(ctx, category, category.CategoryFields)
		if err != nil {
		...
		categoryMap[category.CategorySlug] = CategoryData
	}
...
func GetMarketDataFromCustomCategory(ctx context.Context, category datastruct.CustomCategory, markets []byte) ([]datastruct.Fundamentals, error) {
	// Start tracing and logging
	span, labels := common.GenerateSpan("V2 CustomCategoryService.GetMarketDataFromCustomCategory", ctx)
	defer span.End()
	span.AddEvent("Starting V2 CustomCategoryService.GetMarketDataFromCustomCategory")
	startTime := log.StartTimeL(labels, "Starting V2 CustomCategoryService.GetMarketDataFromCustomCategory")

	// This will help us to map the Category Field to it's correct struct
	// This will help to get access to this struct and loop over it to extract the data we need for Categories Fundamentals build
	switch category.CategoryType {
	case "FT":
		err := json.Unmarshal(markets, &category.Markets)
		if err != nil {
			// Log and return error if JSON unmarshaling fails
			span.SetStatus(codes.Error, "Error unmarshaling Markets JSON in GetMarketDataFromCustomCategory")
			log.EndTime("GetMarketDataFromCustomCategory", startTime, err)
			return nil, err
		}
		// case "NFT":
		// 	err := json.Unmarshal(markets, &category.NFTs)
		// 	if err != nil {
		// 		// Log and return error if JSON unmarshaling fails
		// 		span.SetStatus(codes.Error, "Error unmarshaling Markets JSON in GetMarketDataFromCustomCategory")
		// 		log.EndTime("GetMarketDataFromCustomCategory", startTime, err)
		...
		// 	}
		// case "CATEGORY":
		// 	err := json.Unmarshal(markets, &category.Categories)
		// 	if err != nil {
		// 		// Log and return error if JSON unmarshaling fails
		// 		span.SetStatus(codes.Error, "Error unmarshaling Markets JSON in GetMarketDataFromCustomCategory")
		// 		log.EndTime("GetMarketDataFromCustomCategory", startTime, err)
	...
	// Log successful completion
	log.EndTimeL(labels, "V2 CustomCategoryService.GetMarketDataFromCustomCategory Finished", startTime, nil)
	span.SetStatus(codes.Ok, "success")
	return category.Markets, nil
}
