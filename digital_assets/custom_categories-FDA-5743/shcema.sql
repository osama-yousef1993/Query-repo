...
*Deprecated for V3
*/
CREATE OR REPLACE FUNCTION getCategoriesV2()
RETURNS Table (
  id Text,
  name Text,
  market_cap float,
...
create or replace FUNCTION getExchangeFundamentals()
RETURNS Table(
	name TEXT,
	slug TEXT,
  id text,
...
ALTER TABLE categories_fundamentals ADD COLUMN inactive BOOLEAN DEFAULT false;
...
CREATE OR REPLACE FUNCTION public.getcategoriesfundamentalsV2(
	)
    RETURNS TABLE(
      id text,
      name text,
...
CREATE OR REPLACE FUNCTION public.GetNFTCollection (collectionSlug text)
RETURNS TABLE (
    id TEXT,
...
CREATE OR REPLACE FUNCTION public.GetNFTCollectionWithRank (collectionSlug text)
RETURNS TABLE (
    id TEXT,
    contract_address TEXT,
    asset_platform_id TEXT,
...
* Get all categories list from coingecko_categories
*/
CREATE OR REPLACE FUNCTION public.getcategoriesv3(
	)
    RETURNS TABLE(id text, name text, market_cap double precision, market_cap_change_24h double precision, content text, top_3_coins text[], volume_24h double precision, last_updated timestamp with time zone, markets json, inactive boolean) 
    LANGUAGE 'plpgsql'
AS $BODY$
BEGIN
  RETURN QUERY EXECUTE format('select 
...
CREATE OR REPLACE FUNCTION public.getcategoriesV4(
	)
    RETURNS TABLE(id text, name text, market_cap double precision, market_cap_change_24h double precision, content text, top_3_coins text[], volume_24h double precision, last_updated timestamp with time zone, markets json, inactive boolean, is_highlighted boolean) 
    LANGUAGE 'plpgsql'
AS $BODY$
BEGIN
  RETURN QUERY EXECUTE format('select 
...
*/
CREATE OR REPLACE FUNCTION public.searchtradedassetsbysourcev2(
	source text)
    RETURNS TABLE(symbol text, display_symbol text, name text, slug text, logo text, temporary_data_delay boolean, price_24h double precision, percentage_1h double precision, percentage_24h double precision, percentage_7d double precision, change_value_24h double precision, market_cap double precision, volume_1d double precision, status text, market_cap_percent_change_1d double precision, date_added timestamp with time zone, rank_number bigint, platform_currency_id text, description text) 
    LANGUAGE 'plpgsql'
AS $BODY$
BEGIN
  RETURN QUERY EXECUTE format('select
...
/*
  Gets all information needed for the traded assests search.
*/
CREATE OR REPLACE FUNCTION public.searchtradedassetsbysourcev3(
	source text)
    RETURNS TABLE(symbol text, display_symbol text, name text, slug text, logo text, temporary_data_delay boolean, price_24h double precision, percentage_1h double precision, percentage_24h double precision, percentage_7d double precision, change_value_24h double precision, market_cap double precision, volume_1d double precision, status text, market_cap_percent_change_1d double precision, date_added timestamp with time zone, rank_number bigint, platform_currency_id text, description text, platforms json) 
    LANGUAGE 'plpgsql'
AS $BODY$
BEGIN
  RETURN QUERY EXECUTE format('select
...
CREATE OR REPLACE FUNCTION public.GetFundamentals(
	symbolIN text)
