...
//   - `otelCodes`: Contains status codes for OpenTelemetry tracing.
...
func CombineFundamentalsCG(ctxO context.Context, ticker CoinsMarketResult, highLows PGFundamentalsResult, metadata AssetMetaData, exchangeData []ExchangeBasedFundamentals, allSlugToSymbolMap rfCommon.ConcurrentMap[string, string], oldFundamental Fundamentals) (Fundamentals, error) {
	...
	// todo will use forbes id as slug
	...
	// todo if we generate an empty forbesID we need to send it to remediation table.
...
func MapNFTFundamentalsDataTest(ctx context.Context, nftDescriptions []NftQuestion, nftQuestionsTemplate []FSNFTQuestion, coinsData map[string]string, nftSocialMediaLinks []string, NFTMetaData map[string]FundamentalsNFTSalesData) []NftQuestion {
	_, span := tracer.Start(ctx, "MapNFTFundamentalsData")
	defer span.End()
	span.AddEvent("Start MapNFTFundamentalsData")
	log.Info("Start MapNFTFundamentalsData")
	var NFTSalesDataResult []NftQuestion
	for _, nftSale := range nftDescriptions {
		nftMetaData := NFTMetaData[nftSale.ID]
		if nftMetaData.ID != "" {
			var NFTSalesData NftQuestion
			NFTSalesData.ID = nftSale.ID
			NFTSalesData.NFTQuestion = buildNFTsQuestions(ctx, nftMetaData, nftQuestionsTemplate, coinsData, NFTMetaData, nftSocialMediaLinks)
			NFTSalesDataResult = append(NFTSalesDataResult, NFTSalesData)
		}
	}
...
// 	// Regex to split sentences at periods not part of URLs
// 	reSplit := regexp.MustCompile(`(?s)(.*?)(\.\s+|\.$|\.(?:\s*<a)|\.(?:\s*http))`)
// 	matches := reSplit.FindAllStringSubmatch(text, -1)
...
// 		// Add a delimiter only if it's not the last sentence
// 		if i < len(englishSentences)-1 {
// 			// Check if the next sentence starts with a digit
// 			if len(englishSentences[i+1]) > 0 && (unicode.IsDigit(rune(englishSentences[i+1][0])) || strings.Contains(sentence, "https")) {
// 				result.WriteString(".") // No space for digits
// 			} else {
// 				result.WriteString(". ") // Add space for non-digits
// 			}
// 		}
// 	}
