...
type FundamentalsData struct {
	...
	DateAdded                 time.Time           `postgres:"date_added" json:"date_added"` // The date the asset first was ingested
	Platforms                 map[string]string   `postgres:"platforms" json:"platforms"`   // The date the asset first was ingested
}
...
type TradedAssetsTable struct {
	...
	DateAdded             time.Time         `json:"date_added" firestore:"date_added" postgres:"date_added"`                               // The date the asset first was ingested
	PlatformCurrencyID    string            `json:"platform_currency_id" firestore:"platform_currency_id" postgres:"platform_currency_id"` // The chain of the asset
	Description           string            `json:"description" firestore:"description" postgres:"description"`                            // The chain of the asset
	Platforms             map[string]string `json:"platforms,omitempty" firestore:"platforms" postgres:"platforms"`                        // The chain of the asset
}
...
type FeaturedCategory struct {
	...
	IsHighlighted bool   `json:"is_highlighted" firestore:"is_highlighted"` // Flag indicating that this category belongs on the highlights page
}
...
// This struct contains NFT questions templates
type FSNFTQuestion struct {
	Question      string `json:"question" firestore:"question"`           // NFT template question
	Answer        string `json:"answer" firestore:"answer"`               // NFT template Answer
	QuestionOrder int    `json:"questionOrder" firestore:"questionOrder"` // NFT question order
}
...
// FSNFTSocialMediaLink represents the structure for storing NFT social media links.
// It is used to map data retrieved from Firestore documents into Go structs.
// This struct holds the links that need to be kept in our FAQ section for NFTs, such as Discord, OpenSea, Twitter, etc.
type FSNFTSocialMediaLink struct {
	Link string `json:"link" firestore:"link"` // The social media link associated with an NFT (e.g., discord.gg, opensea.io, x.com, etc.).
}
...
//
// Returns:
//   - *[]byte: Byte data of the retrieved fundamental data.
//   - error: An error if the query fails or data retrieval encounters an issue.
func GetFundamentals(ctxO context.Context, value string, column string, period string) ([]byte, error) {
	ctx, span := tracer.Start(ctxO, "GetFundamentalsFS", trace.WithAttributes(attribute.String("value", value), attribute.String("column", column), attribute.String("period", period)))
	defer span.End()

	result, err := GetFundamentalsPG(ctx, value, column, period)
	if err != nil {
		return nil, err
	}
...
// GetNFTsSocialMediaLinks retrieves social media links for NFTs from a Firestore collection.
// Parameters:
//   - ctx0: The context for managing the request's lifecycle and tracing.
//
// Returns:
//   - []string: A slice of NFT social media links retrieved from the Firestore collection.
//   - error: Returns an error if the operation fails; otherwise, nil.
//
// Details:
//   - Connects to a Firestore client and queries a specified collection for NFT social media links.
//   - Iterates through all documents in the collection, mapping each to an `FSNFTSocialMediaLink` struct.
//   - Handles errors encountered during document mapping and logs them.
//   - Uses OpenTelemetry for tracing and monitors operation success or failure.
func GetNFTsSocialMediaLinks(ctx0 context.Context) ([]string, error) {
	// Initialize the Firestore client for database interactions.
	fs := GetFirestoreClient()

	ctx, span := tracer.Start(ctx0, "GetNFTsSocialMediaLinks")
	defer span.End()

	// Log the start of the operation.
	span.AddEvent("Start Getting NFTs Social Media Links")

	// Define the Firestore collection name dynamically using the ROWY_PREFIX environment variable.
	collectionName := fmt.Sprintf("%s%s", os.Getenv("ROWY_PREFIX"), "nfts_social_media_links")
	...
	for {
		var nftSocialMediaLink FSNFTSocialMediaLink
		...
		// Map the Firestore document data to the FSNFTSocialMediaLink struct.
		if err := doc.DataTo(&nftSocialMediaLink); err != nil {
			// Log the error and update tracing span with failure details.
			log.Error("Error NFTs Social Media Links Data from FS: %s", err)
			span.AddEvent(fmt.Sprintf("Error Community Page NFTs Social Media Links Data from FS: %s", err))
			span.SetStatus(otelCodes.Error, err.Error())
			return nil, err
		}

		// Append the mapped social media link to the result slice.
		nftSocialMediaLinks = append(nftSocialMediaLinks, nftSocialMediaLink.Link)
	}

	// Mark the tracing span as successful upon operation completion.
	span.SetStatus(otelCodes.Ok, "Success")

	// Return the retrieved NFT social media links.
	return nftSocialMediaLinks, nil
}
