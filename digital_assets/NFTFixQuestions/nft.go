...
// Collection name of top Trending NFTs in FS
...
// In the Asset Price Ticker subnav, this is the individual NFT asset of the top 10 trending NFTs
type TrendingNFT struct {
	ID         string   `json:"id,omitempty" postgres:"id,omitempty"`                                                             // (id) for each Crypto Asset
	Name       string   `json:"name,omitempty" postgres:"name,omitempty"`                                                         // name for each Crypto Asset
	Slug       string   `json:"slug,omitempty" bigquery:"slug" firestore:"slug,omitempty" postgres:"slug,omitempty"`              // slug for each Crypto Asset
	Symbol     string   `json:"symbol,omitempty" postgres:"symbol,omitempty"`                                                     // symbol for each Crypto Asset
	Image      string   `json:"image,omitempty" postgres:"image,omitempty"`                                                       // small image for each Crypto Asset
	LargeImage string   `json:"large_image,omitempty" postgres:"large_image,omitempty"`                                           // Large image for each Crypto Asset
	Volume     *float64 `json:"volume_24h_usd,omitempty" postgres:"volume_24h_usd,omitempty"`                                     // Volume over 24h
	Percentage *float64 `json:"volume_24h_percentage_change_usd,omitempty" postgres:"volume_24h_percentage_change_usd,omitempty"` // Percentage change in volume over 24h
	Order      int      `json:"order" bigquery:"order_slug" firestore:"order,omitempty"`                                          // Trending Order from Firestore.
}

// This query will returns the Pag views for NFTs
// COALESCE(SUM(events_data_mart.hits), 0): Ensures that the hits value is returned without nulls, defaulting to 0 if no data is found.
// ((UPPER(events_data_mart.GA_eventAction) = UPPER('click'))): Confirms that the user performed a click action on the NFT name.
// ((UPPER(events_data_mart.GA_eventCategory) = UPPER('forbes digital assets'))): Ensures the click occurred on our pages within the "Forbes Digital Assets" category.
// ((UPPER(events_data_mart.GA_eventLabel) LIKE UPPER('%nftprices{%}'))): Verifies that the click specifically targeted NFTs and not other elements.
var TrendingNFTQuery = `
SELECT
  REPLACE(REGEXP_EXTRACT(events_data_mart.GA_eventLabel, r'nftprices{([^/]+)}'), ' ', '-') AS slug,
  ROW_NUMBER() OVER(ORDER BY COALESCE(SUM(events_data_mart.hits), 0) DESC) AS order_slug
FROM
  api-project-901373404215.DataMart.v_events_datamart AS events_data_mart
WHERE
  ((UPPER(( events_data_mart.GA_eventAction )) = UPPER('click')))
  AND ((UPPER(( events_data_mart.GA_eventCategory )) = UPPER('forbes digital assets')))
  AND ((UPPER(( events_data_mart.GA_eventLabel )) LIKE UPPER('%nftprices{%}')))
GROUP BY
  1
ORDER BY
  2 ASC
