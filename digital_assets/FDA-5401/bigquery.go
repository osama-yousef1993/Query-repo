...
type CoinsMarketResult struct {
	...
	Symbol            string               `bigquery:"Symbol" json:"symbol"`
	Price             bigquery.NullFloat64 `bigquery:"Price" json:"price"`
	CirculatingSupply bigquery.NullFloat64 `bigquery:"CirculatingSupply" json:"circulating_supply"`
	MaxSupply         bigquery.NullFloat64 `bigquery:"MaxSupply" json:"max_supply"`
	MarketCap         bigquery.NullFloat64 `bigquery:"MarketCap" json:"market_cap"`
	Volume            bigquery.NullFloat64 `bigquery:"volume" json:"volume"`
	QuoteCurrency     string               `bigquery:"QuoteCurrency" json:"quote_currency"`
	SOURCE            string               `bigquery:"SOURCE" json:"source"`
	OccurrenceTime    time.Time            `bigquery:"OccurrenceTime" json:"occurrence_time"`
...
func (bq *BQStore) GetCoinsMarketData(ctx0 context.Context) (map[string]CoinsMarketResult, error) {
	...
	WITH market_data AS (
		...
		FROM
			api-project-901373404215.digital_assets.Digital_Asset_MarketData
		)
		SELECT
			ID,
			Name,
...
func (bq *BQStore) ExchangeBasedFundamentalsCG(ctxO context.Context, uuid string) (map[string][]ExchangeBasedFundamentals, error) {
	...
	for {
		...
		fundamentalsResults[fundamentalsResult.Symbol] = append(fundamentalsResults[fundamentalsResult.Symbol], fundamentalsResult)
...
func (bq *BQStore) InsertSearchData(ctx0 context.Context, searchTerm string, results int, tableType string) error {
	...
	queryString := `
		INSERT INTO api-project-901373404215.digital_assets.` + searchTable + ` (search_term, results, created_at)
		values ("` + searchTerm + `", ` + strconv.Itoa(results) + `, CURRENT_TIMESTAMP())`
	...
	if err != nil {
		span.SetStatus(otelCodes.Error, err.Error())
		log.EndTime("InsertSearchData error ", startTime, err)
		return err
	}
...
func (bq *BQStore) GetNFTSalesInfo(ctx context.Context) ([]FundamentalsNFTSalesData, error) {
  ...
  SELECT
    ...
    Avg(volumeUSD) avg_volume_usd_1d,
    Avg(volumeNative) avg_volume_native_1d,
  FROM (
    SELECT
      id,
      occurance_time,
  ...
  SELECT
    ...
    Avg(volumeUSD) avg_volume_usd_7d,
    Avg(volumeNative) avg_volume_native_7d,
  FROM (
    SELECT
      id,
      occurance_time,
  ...
  SELECT
    ...
    Avg(volumeUSD) avg_volume_usd_30d,
    Avg(volumeNative) avg_volume_native_30d,
  FROM (
    SELECT
      id,
      occurance_time,
  ...
  SELECT
    ...
    Avg(volumeUSD) avg_volume_usd_90d,
    Avg(volumeNative) avg_volume_native_90d
  FROM (
    SELECT
      id,
      occurance_time,
  ...
  YTD AS (
    ...
    Avg(volumeUSD) avg_volume_usd_ytd,
    Avg(volumeNative) avg_volume_native_ytd
  FROM (
    SELECT
      id,
      occurance_time,
