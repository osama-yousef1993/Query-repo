...
type PGFundamentalsResult struct {
	...
	PriceOpen24H              bigquery.NullFloat64 `postgres:"price_open_24h" json:"price_open_24h,omitempty" bigquery:"price_open_24h"`
	PriceClose24H             bigquery.NullFloat64 `postgres:"price_close_24h" json:"price_close_24h,omitempty" bigquery:"price_close_24h"`
	PriceOpen7D               bigquery.NullFloat64 `postgres:"price_open_7d" json:"price_open_7d,omitempty" bigquery:"price_open_7d"`
	PriceClose7D              bigquery.NullFloat64 `postgres:"price_close_7d" json:"price_close_7d,omitempty" bigquery:"price_close_7d"`
	PriceOpen30D              bigquery.NullFloat64 `postgres:"price_open_30d" json:"price_open_30d,omitempty" bigquery:"price_open_30d"`
	PriceClose30D             bigquery.NullFloat64 `postgres:"price_close_30d" json:"price_close_30d,omitempty" bigquery:"price_close_30d"`
	PriceOpen1Y               bigquery.NullFloat64 `postgres:"price_open_1y" json:"price_open_1y,omitempty" bigquery:"price_open_1y"`
	PriceClose1Y              bigquery.NullFloat64 `postgres:"price_close_1y" json:"price_close_1y,omitempty" bigquery:"price_close_1y"`
...
func GetCyptoContent(ctxO context.Context, slug string, config model.FDAConfig) (*model.AssetProfile, error) {
	...
	if DATASOURCE == "coingecko" {
		...
		select 
			...
			forbesMetaDataDescription
		from 
			public.getcryptocontentbysource('%s');
					`, slug)
	}
...
// PGGetCoingekoCategories retrieves categories data from the PostgreSQL database.
// It returns a map of category IDs to CategoriesData and an error if any occurs during the query execution.
// public.getCategoriesV2() gets category data from coingeko_categories table
// Parameters:
// - ctx0: The context for the database query.
// - onlyActiveCategories: A boolean flag to filter only active categories.
//
// Returns:
// - map[string]CategoriesData: A map where the key is the category ID and the value is the CategoriesData.
// - error: An error if any occurs during the query execution or scanning of results.
func PGGetCoingekoCategories(ctx0 context.Context, onlyActiveCategories bool) (map[string]CategoriesData, error) {

	ctx, span := tracer.Start(ctx0, "PGGetCoingekoCategories")
	defer span.End()
	var categoryMap = make(map[string]CategoriesData)
	startTime := log.StartTime("Get Categories Query")
	//var categories []CategoriesData
	...
	if err != nil {
		span.SetStatus(codes.Error, "PGGetCoingekoCategories")
		log.EndTime("Get Categories Query", startTime, err)
		return nil, err
	}
	defer queryResult.Close()
	...
	for queryResult.Next() {
		var category CategoriesData
		err := queryResult.Scan(&category.ID, &category.Name, &category.MarketCap, &category.MarketCapChange24H, &category.Content, pq.Array(&category.Top3Coins), &category.Volume24H, &category.UpdatedAt, (*coinsMarketResultResult)(&category.Markets), &category.Inactive)
		if err != nil {
			span.SetStatus(codes.Error, "PGGetCoingekoCategories scan error")
			log.EndTime("Get Categories Query", startTime, err)
			return nil, err
		}
		categoryMap[category.ID] = category
	}
	log.EndTime("PGGetCoingekoCategories Query", startTime, nil)
	span.SetStatus(codes.Ok, "success")
	return categoryMap, nil
}
