...
type Fundamentals struct {
	...
	SlugOverride              string                      `postgres:"slug_override" firestore:"slug_override"` // this can be set in postgres to override the assigned slug through our automated process
}
...
type ExchangeBasedFundamentals struct {
	ForbesID                     string              `json:"forbes_id" bigquery:"forbes_id"`
...
func MapChartDataToFundamental(ctxO context.Context, tsResults []TimeSeriesResultPG, fundamental Fundamentals, openClosePrices []OpenCloseAsset) (Fundamentals, TimeSeriesResultPG) {
	...
	for _, cd := range tsResults {
		if cd.Symbol == fundamental.Symbol {
			...
			cd.ForbesID = fundamental.ForbesID
			chartData = cd
		}
	}
...
func extractHighLowValues(ctxO context.Context, fundamental *Fundamentals, highLows PGFundamentalsResult) Fundamentals {
	...
	if highLows.High.Valid && highLows.Low.Valid {
	...
	fundamental.Price7D = &highLows.PriceOpen7D.Float64
	...
	fundamental.Percentage7D = CalculatePercentageChange(&highLows.PriceOpen7D.Float64, &highLows.PriceClose7D.Float64)

	// 30D High/Low
	fundamental.High30D = &highLows.High30D.Float64
	fundamental.Low30D = &highLows.Low30D.Float64
	fundamental.Price30D = &highLows.PriceOpen30D.Float64
	...
	fundamental.Percentage30D = CalculatePercentageChange(&highLows.PriceOpen30D.Float64, &highLows.PriceClose30D.Float64)

	// 1Y High/Low
	fundamental.High1Y = &highLows.High1Y.Float64
	fundamental.Low1Y = &highLows.Low1Y.Float64
	fundamental.Price1Y = &highLows.PriceOpen1Y.Float64
	...
	fundamental.Percentage1Y = CalculatePercentageChange(&highLows.PriceOpen1Y.Float64, &highLows.PriceClose1Y.Float64)
...
func CombineFundamentalsCG(ctxO context.Context, ticker CoinsMarketResult, highLows PGFundamentalsResult, metadata AssetMetaData, exchangeData []ExchangeBasedFundamentals) (Fundamentals, error) {
	_, span := tracer.Start(ctxO, "CombineFundamentalsCG")
	defer span.End()

	span.AddEvent("Start Combine Fundamentals")
	var fundamental Fundamentals
	// Basic Data from PG Assets Metadata
	fundamental.Slug = ticker.ForbesID
	fundamental.Logo = metadata.LogoURL
	fundamental.OriginalSymbol = metadata.OriginalSymbol

	// Basic Data from tickers
	fundamental.ForbesID = ticker.ForbesID
...
// removeNonEnglishSentences filters out non-English sentences from the input text.
// It follows these steps:
// 1. Identifies and replaces URLs in the text with a temporary placeholder to prevent them from being split.
// 2. Splits the text into individual sentences, ignoring periods inside URL placeholders.
// 3. Uses a regular expression to check for non-English characters and removes sentences containing them.
// 4. Rebuilds the text with proper punctuation between sentences.
// 5. Replaces the placeholders back with the original URLs.
// 6. Returns the cleaned-up text containing only English sentences with correct punctuation.
//
// Parameters:
// - text: The input string containing sentences in different languages and URLs.
//
// Returns:
// - string: The cleaned-up string containing only English sentences, with URLs properly restored.
func removeNonEnglishSentences(text string) string {
	// Regex to find URLs and temporarily replace them with placeholders
	urlRegex := regexp.MustCompile(`(<a\s+[^>]*href="(https?://[^"]+)"[^>]*>.*?</a>|https?://\S+)`)
	...
	textWithPlaceholders := urlRegex.ReplaceAllString(text, placeholder)

	// Split the text into sentences (but ignore periods inside placeholders)
	sentences := strings.Split(textWithPlaceholders, ".")

	// Regex to match any non-English characters
	...
	for _, sentence := range sentences {
		...
		if !nonEnglishRegex.MatchString(trimmed) {
			englishSentences = append(englishSentences, trimmed)
		}
	}

	// Rebuild the text with proper punctuation
	var result strings.Builder
	for i, sentence := range englishSentences {
		result.WriteString(sentence)

		// Add a "." unless it's the last sentence
		if i < len(englishSentences)-1 {
			// Check if the next sentence starts with a digit or is part of a URL placeholder
			nextSentence := englishSentences[i+1]
			isDigit := len(nextSentence) > 0 && unicode.IsDigit(rune(nextSentence[0]))
			isURLPlaceholder := strings.Contains(sentence, placeholder)

			if isDigit || isURLPlaceholder {
				result.WriteString(".") // No space
			} else {
				result.WriteString(". ") // Add space
			}
		}
	}
	...
	for _, url := range urlMatches {
		finalResult = strings.Replace(finalResult, placeholder, url, 1)
	}
