...
func (bq *BQStore) GetCoinsMarketData(ctx0 context.Context) (map[string]CoinsMarketResult, error) {
	...
	WITH market_data AS (
		SELECT
			...
			ROW_NUMBER() over (PARTITION BY forbes_id order by Occurance_Time desc) as row_num
		FROM
...
func (bq *BQStore) BuildHighLowFundamentalsCG(ctx0 context.Context, uuid string) ([]PGFundamentalsResult, error) {
	...
	query := bq.Query(`
		...
		allTime AS (
			SELECT
				MIN(all_time_low) AS all_time_low,
				MAX(all_time_high) AS all_time_high,
				forbes_id,
				SUM(COALESCE(num_markets, 0)) AS number_of_active_market_pairs
			FROM (
			SELECT
		...
		SELECT
			...
			CAST(oneDay.close_value.Volume AS FLOAT64) AS volume_close_24h,
      			CAST(oneDay.open_value.Price AS FLOAT64) AS price_open_24h,
			      CAST(oneDay.close_value.Price AS FLOAT64) AS price_close_24h,
			...
			CAST(oneDay.close_value.Volume AS FLOAT64) AS volume_close_7d,
      			CAST(sevenDays.open_value.Price AS FLOAT64) AS price_open_7d,
			      CAST(oneDay.close_value.Price AS FLOAT64) AS price_close_7d,
			...
			CAST(oneDay.close_value.Volume AS FLOAT64) AS volume_close_30d,
      			CAST(thirtyDays.open_value.Price AS FLOAT64) AS price_open_30d,
			      CAST(oneDay.close_value.Price AS FLOAT64) AS price_close_30d,
			...
			CAST(oneDay.close_value.Volume AS FLOAT64) AS volume_close_1y,
      			CAST(oneYear.open_value.Price AS FLOAT64) AS price_open_1y,
			      CAST(oneDay.close_value.Price AS FLOAT64) AS price_close_1y,
