...
func GetCoinGeckoMetaData(ctx context.Context) (map[string]AssetMetaData, error) {
	...
	FROM
		(SELECT ID,
				...
				DESCRIPTION,
				LOGO_URL
			FROM PUBLIC.COINGECKO_ASSET_METADATA) METADATA
	LEFT JOIN
		(SELECT ID AS FORBES_ID,
				COINGECKO_ID
			FROM PUBLIC.FORBES_ASSETS) FORBES_ASSETS ON FORBES_ASSETS.COINGECKO_ID = METADATA.ID;
	`)
...
func GetOpenClosePrice(labels map[string]string, ctxO context.Context) ([]OpenCloseAsset, error) {
	...
	FROM
		(SELECT SYMBOL,
				...
				'open',
				CAST(PRICES::JSON -> 0 ->> 'Price' AS float),
				'close',
				CAST(PRICES::JSON -> -1 ->> 'Price' AS float),
				'interval', interval)) AS LIST
			FROM NOMICS_CHART_DATA
			WHERE "assetType" = 'FT'
			GROUP BY SYMBOL) CHARTS
	LEFT JOIN
		(SELECT ID AS FORBES_ID,
				COINGECKO_ID
			FROM PUBLIC.FORBES_ASSETS
		) FORBES_ASSETS 
	ON FORBES_ASSETS.COINGECKO_ID = CHARTS.SYMBOL`
