...
type NFTsPriceService interface {
	...
	SearchChainsTerm(ctx context.Context, searchTerm string, paginate dto.Paginate) (*[]byte, int, error)
}

type nftsPriceService struct {
	dao                     repository.DAO
...
func NewNFTsService(dao repository.DAO) NFTsPriceService {
	return &nftsPriceService{dao: dao, Category: dto.Nft, Lock: &sync.Mutex{}, DefaultFuzzySearchLimit: 100, Datasource: "calculated", SearchType: dto.Fuzzy}
}
...
func (n *nftsPriceService) GetChainsList(ctx context.Context) ([]datastruct.NFTChain, error) {
...
func (n *nftsPriceService) SearchTerm(ctx context.Context, searchTerm string, paginate dto.Paginate) (*[]byte, int, error) {
...
func (n *nftsPriceService) SearchChainsTerm(ctx context.Context, searchTerm string, paginate dto.Paginate) (*[]byte, int, error) {
	...
	if !isChain {
		err := errors.New("V2 NFTsCache.SearchTerm Failed to Assert Type []datastruct.CategoryFundamental ")
		span.SetStatus(codes.Error, err.Error())
		log.ErrorL(labels, err.Error())
		log.EndTimeL(labels, "V2 NFTsCache.SearchTerm", startTime, err)
		return nil, 0, err
	...
	nftsChains, words := n.BuildNFTs(ctx, nftsChain)
	searchTerm = strings.ToLower(searchTerm)
...
func (n *nftsPriceService) BuildCache(ctx0 context.Context, buildType string) error {
...
func (n *nftsPriceService) BuildNFTs(ctx0 context.Context, nfts []datastruct.NFTPrices) (map[string][]datastruct.NFTPrices, []string) {
	_, span := tracer.Start(ctx0, "buildFTDictionary")
	defer span.End()
...
// - map[string][]datastruct.NFTPrices this is a map of all NFTs value that we have.
// - []string this an array of words we will use to in fuzzy search.
func (n *nftsPriceService) BuildNFTsChains(ctx0 context.Context, nfts []datastruct.NFTPrices) (map[string][]datastruct.NFTPrices, []string) {
...
func (n *nftsPriceService) PaginateSortNFTs(ctx context.Context, allNFTs interface{}, paginate dto.Paginate, ignoreInitialAssets int) interface{} {
	span, labels := common.GenerateSpan("V2 NFTsCache.SearchTerm", ctx)
...
func (n *nftsPriceService) RemoveDuplicateNFTs(ctx0 context.Context, nfts []datastruct.NFTPrices) []datastruct.NFTPrices {
