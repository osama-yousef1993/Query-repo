...
func CalculatePercentageChange(priceThen bigquery.NullFloat64, priceNow bigquery.NullFloat64) *float64 {
	var newPercentChange float64 = 0
	if priceThen.Valid && priceNow.Valid {
...
func MapNFTFundamentalsData(ctx context.Context, NFTSalesData []FundamentalsNFTSalesData, NFTVolumeData, NFTFloorPriceData, NFTPctData, NFTMetaData map[string]FundamentalsNFTSalesData, nftQuestionsTemplate []FSNFTQuestion, coinsData map[string]string) []FundamentalsNFTSalesData {
...
func buildNFTsQuestions(ctx context.Context, NFTMetaData FundamentalsNFTSalesData, nftQuestionsTemplate []FSNFTQuestion, coinsData map[string]string, nftsMetaData map[string]FundamentalsNFTSalesData) []NFTQuestion {
	...
	NFTMetaData.Description = replaceLinks(NFTMetaData.Description, nftsMetaData, coinsData, NFTMetaData.ID)
...
func replaceLinks(description string, nftsMetaData map[string]FundamentalsNFTSalesData, coinsData map[string]string, nftId string) string {
		...
		if len(submatches) == 3 {
			...
			if len(parts) == 2 {
				if strings.Contains(cleanSlug(text), cleanSlug(parts[1])) {
					...
					return fmt.Sprintf("<a href=\"https://www.forbes.com/digital-assets/nfts/%s\" target=\"_blank\">%s</a>", nftSlug.Slug, submatches[2])
	...
	// All Social media that will be exist in coingecko description
	...
	// Remove all foreign links from description.
	// And this will exclude the Social media links.
