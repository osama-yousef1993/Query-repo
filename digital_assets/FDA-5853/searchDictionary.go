...
func RebuildCache(ctx context.Context, ignoreLock bool) error {
	...
	categories, err := PGGetCategories(ctx0, true)
	if err != nil {
		return err
	}
...
func SearchTerm(ctx context.Context, searchTerm string, dictionaryCategory DictionaryCategory, paginate Paginate) ([]byte, error) {
	...
	*/
	...
	if err == nil {
		// send search term for storage and move on
		go func(ctx context.Context, searchTerm string, totalAssets int, dataType string) {
			ReportSearchTerm(ctx, searchTerm, totalAssets, dataType)
		}(ctx0, searchTerm, totalAssets, "asset")
	}
	if err != nil {
...
func ReportSearchTerm(ctx context.Context, searchTerm string, totalAssets int, dataType string) {
	bqs, _ := NewBQStore()
	bqs.InsertSearchData(ctx, searchTerm, totalAssets, "asset")
}
...
func buildCategoriesDictionary(ctx0 context.Context, assets []TradedAssetsTable, categories map[string]CategoriesData) {
		...
		if category.ID != "" {
			...
			for _, asset := range assets {
				if asset.Symbol != "" && assetSymbols[asset.Symbol] {
					if i > 0 && categoryKey == "new-cryptocurrencies" {
						var x float64 = 0
						asset.Percentage1H = &x
						i--
					}
	...
	if paginate.SortBy == "name" {
		...
		sort.Slice(assets, func(i, j int) bool { // sort the assets
			...
			case "rank":
				result = compareFloat(assets[i].MarketCap, assets[j].MarketCap, paginate.Direction)
			case "percentage":
				result = compareFloatTest(assets[i].Percentage, assets[j].Percentage, paginate.Direction)
			case "percentage_1h":
			...
			case "date_added":
				result = compareInt64(assets[i].DateAdded.UnixMilli(), assets[j].DateAdded.UnixMilli(), paginate.Direction)
			}
			return result
		})
	}
...
// compare string values for the sort.Slices function
func compareString(val1 string, val2 string, direction string) bool {
	if val1 == "" || val2 == "" {
		return false
	}
	if direction == "desc" {
...
func compareFloatTest(val1, val2 *float64, direction string) bool {
	epsilon := 1e-9
	// Handle nil cases
	if val1 == nil && val2 == nil {
		return false
	...
	// If one value is nil, handle based on direction
	if val1 == nil {
	...
	// Use nearlyEqual for float comparisons
	if direction == "desc" {
		return !nearlyEqual(*val1, *val2, epsilon) && *val1 > *val2
	}
...
// compare float values for the sort.Slices function
func compareInt64(val1 int64, val2 int64, direction string) bool {
	if direction == "desc" {
		return val1 > val2
	}
	return val1 < val2
