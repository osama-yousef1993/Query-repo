	...
	for _, openClose := range openClosePrices {
		if openClose.Symbol == fundamental.Symbol {
			...
			if (&openClose).Price24H.Close != nil && (&openClose).Price24H.Open != nil {
				...
				fundamental.Percentage24h = CalculatePercentagePointers((&openClose).Price24H.Open, (&openClose).Price24H.Close)
				fundamental.Price7D = openClose.Price7D.Open
				fundamental.Percentage7D = CalculatePercentagePointers((&openClose).Price7D.Open, (&openClose).Price7D.Close)
				fundamental.Price30D = openClose.Price30D.Open
				fundamental.Percentage30D = CalculatePercentagePointers((&openClose).Price30D.Open, (&openClose).Price30D.Close)
				fundamental.Price1Y = openClose.Price1Y.Open
				fundamental.Percentage1Y = CalculatePercentagePointers((&openClose).Price1Y.Open, (&openClose).Price1Y.Close)
			} else {
...
func MapCategoryFundamental(ctxO context.Context, category CategoriesData, assets *[]Fundamentals, categoryFundamental24hList CategoryFundamental, chartCategoriesData24hrResults []TimeSeriesResultPG) (CategoryFundamental, TimeSeriesResultPG) {
	...
	var categoryChart TimeSeriesResultPG
...
func CombineExchangeDataCG(ctxO context.Context, exchangeData []ExchangeBasedFundamentals, profiles map[string]model.CoingeckoExchangeMetadata, exchangeProfiles map[string]model.ExchangeProfile) ([]ExchangeBasedFundamentals, float64) {
	...
	for _, exchange := range exchangeData {
		if profile, ok := profiles[exchange.Exchange]; ok {
			...
			exchange.Slug = strings.ToLower(strings.ReplaceAll(profile.Name, " ", "-"))
...
func extractHighLowValues(ctxO context.Context, fundamental *Fundamentals, highLows PGFundamentalsResult) Fundamentals {
	...
	if highLows.High.Valid == true && highLows.Low.Valid == true {
		...
		fundamental.Percentage24h = CalculatePercentageNullFloat64(highLows.Low, highLows.High)
	}

	// 1D Data
	fundamental.Nomics.PercentageVolume1D = CalculatePercentageNullFloat64(highLows.VolumeOpen24H, highLows.VolumeClose24H)
	fundamental.MarketCapPercentChange1D = CalculatePercentageNullFloat64(highLows.MarketCapOpen24H, highLows.MarketCapClose24H)

	// 7D Data
	fundamental.High7D = &highLows.High7D.Float64
	fundamental.Low7D = &highLows.Low7D.Float64
	fundamental.MarketCapPercentChange7D = CalculatePercentageNullFloat64(highLows.MarketCapOpen7D, highLows.MarketCapClose7D)
	fundamental.Nomics.PercentageVolume7D = CalculatePercentageNullFloat64(highLows.VolumeOpen7D, highLows.VolumeClose7D)

	// 30D High/Low
	fundamental.High30D = &highLows.High30D.Float64
	fundamental.Low30D = &highLows.Low30D.Float64
	fundamental.MarketCapPercentChange30D = CalculatePercentageNullFloat64(highLows.MarketCapOpen30D, highLows.MarketCapClose30D)
	fundamental.Nomics.PercentageVolume30D = CalculatePercentageNullFloat64(highLows.VolumeOpen30D, highLows.VolumeClose30D)

	// 1Y High/Low
	fundamental.High1Y = &highLows.High1Y.Float64
	fundamental.Low1Y = &highLows.Low1Y.Float64
	fundamental.MarketCapPercentChange1Y = CalculatePercentageNullFloat64(highLows.MarketCapOpen1Y, highLows.MarketCapClose1Y)
	fundamental.Nomics.PercentageVolume1Y = CalculatePercentageNullFloat64(highLows.VolumeOpen1Y, highLows.VolumeClose1Y)
	...
	fundamental.MarketCapPercentChangeYTD = CalculatePercentageNullFloat64(highLows.MarketCapOpenYTD, highLows.MarketCapCloseYTD)
	fundamental.Nomics.PercentageVolumeYTD = CalculatePercentageNullFloat64(highLows.VolumeOpenYTD, highLows.VolumeCloseYTD)
	fundamental.PercentageYTD = CalculatePercentageNullFloat64(highLows.PriceOpenYTD, highLows.PriceCloseYTD)
	...
	fundamental.MarketCapPercentChange1H = CalculatePercentageNullFloat64(highLows.MarketCapOpen1H, highLows.MarketCapClose1H)
	fundamental.Nomics.PercentageVolume1H = CalculatePercentageNullFloat64(highLows.VolumeOpen1H, highLows.VolumeClose1H)
	fundamental.Percentage1H = CalculatePercentageNullFloat64(highLows.PriceOpen1H, highLows.PriceClose1H)
...
func CalculatePercentageChange(priceThen float64, priceNow float64) *float64 {
	var newPercentChange7D float64
	if priceThen > 0 {
		newPercentChange7D = (priceNow - priceThen) / priceThen
		return &newPercentChange7D
	}
	return &newPercentChange7D
}

// It adds an additional validation for the input types of `bigquery.NullFloat64` before going for the percent calculation
func CalculatePercentageNullFloat64(priceThen bigquery.NullFloat64, priceNow bigquery.NullFloat64) *float64 {
	if priceThen.Valid == true && priceNow.Valid == true {
		return CalculatePercentageChange(priceThen.Float64, priceNow.Float64)
	}
	return nil
}

// It adds an additional validation for the input types of *float64 before going for the percent calculation
func CalculatePercentagePointers(priceThen *float64, priceNow *float64) *float64 {
	if priceThen != nil && priceNow != nil {
		return CalculatePercentageChange(*priceThen, *priceNow)
	}
	return nil
}
...
func CombineExchanges(ctx0 context.Context, exchangeMetadata model.CoingeckoExchangeMetadata, exchangeData ExchangeResults, exchangeProfiles map[string]model.ExchangeProfile) (ExchangeFundamentals, error) {
	...
	exchange.Slug = strings.ToLower(strings.ReplaceAll(exchangeData.Name, " ", "-"))
...
// will map all NFTVolumeData, NFTVolumePctData and NFTFloorPriceData to NFTSalesData
// It Takes (ctx context.Context, NFTSalesData []FundamentalsNFTSalesData,  NFTVolumeData, NFTFloorPriceData, NFTVolumePctData map[string]FundamentalsNFTSalesData)
// Returns []FundamentalsNFTSalesData
//
// It will map all NFTs Volume data and Floor Price Data that we calculate it from Bigquery ad map it to each NFT
// Returns []FundamentalsNFTSalesData with all fields to be add it to PG
func MapNFTFundamentalsData(ctx context.Context, NFTSalesData []FundamentalsNFTSalesData, NFTVolumeData, NFTFloorPriceData, NFTVolumePctData, NFTMetaData map[string]FundamentalsNFTSalesData, nftQuestionsTemplate []FSNFTQuestion, coinsData map[string]string) []FundamentalsNFTSalesData {
	...
	var questionLinks []QuestionsLinks
	for _, nftSale := range NFTSalesData {
		var questionLink []QuestionsLinks
		nftVolumePct := NFTVolumePctData[nftSale.ID]
		...
		nftSale.PctChangeVolumeNative1d = nftVolumePct.PctChangeVolumeNative1d
		nftSale.PctChangeVolumeNative7d = nftVolumePct.PctChangeVolumeNative7d
		nftSale.PctChangeVolumeNative30d = nftVolumePct.PctChangeVolumeNative30d
		nftSale.PctChangeVolumeNative90d = nftVolumePct.PctChangeVolumeNative90d
		nftSale.PctChangeVolumeNativeYtd = nftVolumePct.PctChangeVolumeNativeYtd

		nftSale.PctChangeVolumeUSD1d = nftVolumePct.PctChangeVolumeUSD1d
		nftSale.PctChangeVolumeUSD7d = nftVolumePct.PctChangeVolumeUSD7d
		nftSale.PctChangeVolumeUSD30d = nftVolumePct.PctChangeVolumeUSD30d
		nftSale.PctChangeVolumeUSD90d = nftVolumePct.PctChangeVolumeUSD90d
		nftSale.PctChangeVolumeUSDYtd = nftVolumePct.PctChangeVolumeUSDYtd
		...
		nftSale.NFTQuestion, questionLink = buildNFTsQuestions(ctx, nftMetaData, nftQuestionsTemplate, coinsData)

		NFTSalesDataResult = append(NFTSalesDataResult, nftSale)
		questionLinks = append(questionLinks, questionLink...)
	}

	file, _ := json.MarshalIndent(questionLinks, "", " ")
	_ = os.WriteFile("questionLinks.json", file, 0644)
	span.SetStatus(otelCodes.Ok, "Success")
...
type QuestionsLinks struct {
	ForbesLink    string `json:"forbesLink"`
	CoingeckoLink string `json:"coingeckoLink"`
}

func buildNFTsQuestions(ctx context.Context, NFTMetaData FundamentalsNFTSalesData, nftQuestionsTemplate []FSNFTQuestion, coinsData map[string]string) ([]NFTQuestion, []QuestionsLinks) {
	_, span := tracer.Start(ctx, "buildNFTsQuestions")
	defer span.End()
	var questionsLinks []QuestionsLinks
	span.AddEvent("Start buildNFTsQuestions")
	NFTMetaData.Description, questionsLinks = replaceLinks(NFTMetaData.Description, NFTMetaData.Slug, coinsData)

	/*
			(?:\r?\n---\r?\n)|            # Matches horizontal lines
		    (\*\*|\*)|                    # Matches ** and * for bold and italic
		    (#\s*)|                       # Matches # for headers
	...
	*/
	...
	if strings.Contains(NFTMetaData.Description, "<h3 dir=\"ltr\">") || strings.Contains(NFTMetaData.Description, "<h3>") {
		...
		cleanDescription := cleanupRegex.ReplaceAllString(NFTMetaData.Description, "")
		cleanDescription = strings.ReplaceAll(cleanDescription, "\\n", " ")
		NFTMetaData.Description = strings.TrimSpace(cleanDescription)

		if len(strings.Fields(NFTMetaData.Description)) < 10 {
	...
	span.SetStatus(otelCodes.Ok, "Success")
	return nftQuestions, questionsLinks

}

func replaceLinks(description string, nftSlug string, coinsData map[string]string) (string, []QuestionsLinks) {
	// Replace NFT links
	var questionLinks []QuestionsLinks
	nftRegex := regexp.MustCompile(`https://www\.coingecko\.com/en/nft/([^"]+)`)
	description = nftRegex.ReplaceAllStringFunc(description, func(match string) string {
		var questionLink QuestionsLinks
		parts := strings.SplitN(match, "nft/", 2)
		questionLink.CoingeckoLink = match
		if len(parts) == 2 {
			questionLink.ForbesLink = "https://dev84.simple-site.forbes.com/digital-assets/nfts/" + nftSlug
			questionLinks = append(questionLinks, questionLink)
			return "https://dev84.simple-site.forbes.com/digital-assets/nfts/" + nftSlug
		}
		return match
	})

	// Replace coin links
	coinRegex := regexp.MustCompile(`https://www\.coingecko\.com/en/coins/([^"]+)`)
	description = coinRegex.ReplaceAllStringFunc(description, func(match string) string {
		var questionLink QuestionsLinks
		parts := strings.SplitN(match, "coins/", 2)
		questionLink.CoingeckoLink = match
		if len(parts) == 2 {
			slug := coinsData[parts[1]]
			questionLink.ForbesLink = "https://www.forbes.com/digital-assets/assets/" + slug
			questionLinks = append(questionLinks, questionLink)
			return "https://www.forbes.com/digital-assets/assets/" + slug
		}
		return match
	})

	// Replace learn links
	learnRegex := regexp.MustCompile(`https://www\.coingecko\.com/learn/([^"]+)`)
	description = learnRegex.ReplaceAllStringFunc(description, func(match string) string {
		var questionLink QuestionsLinks
		parts := strings.SplitN(match, "learn/", 2)
		questionLink.CoingeckoLink = match
		if len(parts) == 2 {
			questionLink.ForbesLink = "https://www.forbes.com/digital-assets/learn/"
			questionLinks = append(questionLinks, questionLink)
			return "https://www.forbes.com/digital-assets/learn/"
		}
		return match
	})

	// Replace research links
	researchRegex := regexp.MustCompile(`https://www\.coingecko\.com/research/([^"]+)`)
	description = researchRegex.ReplaceAllStringFunc(description, func(match string) string {
		var questionLink QuestionsLinks
		parts := strings.SplitN(match, "research/", 2)
		questionLink.CoingeckoLink = match
		if len(parts) == 2 {
			questionLink.ForbesLink = "https://www.forbes.com/digital-assets/research/"
			questionLinks = append(questionLinks, questionLink)
			return "https://www.forbes.com/digital-assets/research/"
		}
		return match
	})

	// Replace business links
	glossaryRegex := regexp.MustCompile(`https://www\.coingecko\.com/glossary/([^"]+)`)
	description = glossaryRegex.ReplaceAllStringFunc(description, func(match string) string {
		var questionLink QuestionsLinks
		parts := strings.SplitN(match, "business/", 2)
		questionLink.CoingeckoLink = match
		if len(parts) == 2 {
			questionLink.ForbesLink = "https://www.forbes.com/digital-assets/learn/"
			questionLinks = append(questionLinks, questionLink)
	...
	return description, questionLinks
}
...
func BuildQuestionTemplate(ctx context.Context, nftMetaData FundamentalsNFTSalesData, isShort bool, nftQuestionsTemplate []FSNFTQuestion) []NFTQuestion {
	...
	// questions := []string{"What is %s?", "Where can I buy %s?"}
	// answers := []string{"%s is a NFT collection with a total of %v of NFTs minted, held by %d unique owners.", "You can buy, sell and trade %s on %s"}
	...
	for _, question := range nftQuestionsTemplate {
		...
		case 1: // First question
			...
			if !isShort && !isNonEnglish && !isMorseCode {
				...
				s := "%s is a unique digital asset built currently traded on the %s blockchain. This NFT Collection was first minted in %d. Each NFT (Non-Fungible Token) represents ownership of a specific item, piece of content, or collectible. %s NFTs are one-of-a-kind, making them unique and valuable to collectors.There are currently %v within the %s collection."
				// fmt.Print(s)
				// answer = fmt.Sprintf(question.Answer, nftMetaData.Name, nftMetaData.TotalSupply, nftMetaData.NumberOfUniqueAddresses)
				answer = fmt.Sprintf(s, nftMetaData.Name, nftMetaData.AssetPlatformId, 2024, nftMetaData.Name, nftMetaData.TotalSupply, nftMetaData.Name)
			}
		case 2: // Second question
			...
			if len(tickers) > 1 {
				lastExplorers = tickers[len(tickers)-1]
				lastExplorers = fmt.Sprintf(" and %s", lastExplorers)

			}
			ticker := fmt.Sprintf("%s%s", strings.Join(tickers, ","), lastExplorers)
