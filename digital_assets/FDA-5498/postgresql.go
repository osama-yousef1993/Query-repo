...
func UpsertCategoryFundamentalsPG(ctx0 context.Context, allFundamentals *[]CategoryFundamental, labels map[string]string) error {
	...
	valueArgs := make([]interface{}, 0, len(fundamentals)*21)
	var i = 0
	...
	for y := 0; y < len(fundamentals); y++ {
		...
		var valString = fmt.Sprintf("($%d,$%d,$%d,$%d,$%d,$%d,$%d,$%d,$%d,$%d,$%d,$%d,$%d,$%d,$%d,$%d,$%d,$%d,$%d,$%d,$%d)", i*21+1, i*21+2, i*21+3, i*21+4, i*21+5, i*21+6, i*21+7, i*21+8, i*21+9, i*21+10, i*21+11, i*21+12, i*21+13, i*21+14, i*21+15, i*21+16, i*21+17, i*21+18, i*21+19, i*21+20, i*21+21)
		valueString = append(valueString, valString)
...
func PGGetCategories(ctx0 context.Context, onlyActiveCategories bool) ([]CategoriesData, error) {
...
func UpsertNFTSalesData(ctx0 context.Context, nftdata *[]FundamentalsNFTSalesData) error {
	...
	totalFields := 97 //total number of columns in the postgres collection
...
func GetNFTMetaData(ctx0 context.Context) (map[string]FundamentalsNFTSalesData, error) {
	...
	queryResult, err := pg.QueryContext(ctx, `
		SELECT 
			...
			number_of_unique_addresses, 
			total_supply,
			COALESCE(tickers, '[]'::JSON ) as tickers
	...
	for queryResult.Next() {
		...
		err := queryResult.Scan(&nftMetaData.ID, &nftMetaData.Name, &nftMetaData.AssetPlatformId, &nftMetaData.Slug, &nftMetaData.Description, &nftMetaData.NumberOfUniqueAddresses, &nftMetaData.TotalSupply, (*NFTTickerResult)(&nftMetaData.Tickers))
		if err != nil {
...
func GetAllNFTs(ctx0 context.Context) ([]NftCollection, error) {
	pg := PGConnect()
	...
	queryResult, err := pg.QueryContext(ctx, `
		SELECT 
			...
			description,
			slug,
			COALESCE(questions, '[]'::JSON ) as questions
		FROM public.nftdatalatest
		`)
	...
	var nftsMetaData []NftCollection
	for queryResult.Next() {
		var nftMetaData NftCollection
		err := queryResult.Scan(&nftMetaData.ID, &nftMetaData.Name, &nftMetaData.NumberOfUniqueAddresses, &nftMetaData.Description, &nftMetaData.Slug, (*NFTQuestionResult)(&nftMetaData.NFTQuestion))
		if err != nil {
			span.SetStatus(codes.Error, "GetNFTMetaData scan error")
			log.EndTime("Get  NFT MetaData Query", startTime, err)
			return nil, err
		}
		nftsMetaData = append(nftsMetaData, nftMetaData)
	}
	log.EndTime("Get  NFT MetaData Query", startTime, nil)
	span.SetStatus(codes.Ok, "success")
	return nftsMetaData, nil
