...
func MapNFTFundamentalsData(ctx context.Context, NFTSalesData []FundamentalsNFTSalesData, NFTVolumeData, NFTFloorPriceData, NFTVolumePctData, NFTMetaData map[string]FundamentalsNFTSalesData, nftQuestionsTemplate []FSNFTQuestion, coinsData map[string]string) []FundamentalsNFTSalesData {
	...
	for _, nftSale := range NFTSalesData {
		...
		nftSale.NFTQuestion = buildNFTsQuestions(ctx, nftMetaData, nftQuestionsTemplate, coinsData, NFTMetaData)
...
func removeNonEnglishSentences(text string) string {
	...
	// Add a period at the end if the result is not empty
	if result != "" {
		result += "."
	}
...
func buildNFTsQuestions(ctx context.Context, NFTMetaData FundamentalsNFTSalesData, nftQuestionsTemplate []FSNFTQuestion, coinsData map[string]string, nftsMetaData map[string]FundamentalsNFTSalesData) []NFTQuestion {
	_, span := tracer.Start(ctx, "buildNFTsQuestions")
	defer span.End()
	span.AddEvent("Start buildNFTsQuestions")
	NFTMetaData.Description = replaceLinks(NFTMetaData.Description, nftsMetaData, coinsData)
...
// replaceLinks
// Takes (description string, nftsMetaData map[string]FundamentalsNFTSalesData, coinsData map[string]string)
// Returns string
//
// It will replace Coingecko links with Forbes Links and returns the results for it
// Returns string this will be the new link for Forbes
func replaceLinks(description string, nftsMetaData map[string]FundamentalsNFTSalesData, coinsData map[string]string) string {
	// Replace NFT links
	...
	description = nftRegex.ReplaceAllStringFunc(description, func(match string) string {
		...
		if len(parts) == 2 {
			...
			return "https://www.forbes.com/digital-assets/nfts/" + nftSlug.Slug
		}
		return match
	})
	...
	// Create a regex pattern for social media links
	// socialMediaPattern := "(?i)^https?://(?:www\\.)?(?:" + strings.Join(socialMediaDomains, "|") + ")/"

	// Check all Social media links from nft description
	// socialMediaRegex := regexp.MustCompile(`(?i)^https?://(?:www\.)?(?:twitter\.com|x\.com|facebook\.com|tiktok\.com|instagram\.com|linkedin\.com|opensea\.io|niftygateway\.com|discord\.gg)/`)
	socialMediaRegex := regexp.MustCompile("(?i)^https?://(?:www\\.)?(?:" + strings.Join(socialMediaDomains, "|") + ")/")
	description = socialMediaRegex.ReplaceAllStringFunc(description, func(match string) string {
		return match
	})

	// socialMediaRegex := regexp.MustCompile(`(?i)^https?://(?:www\.)?(?:twitter\.com|x\.com|facebook\.com|instagram\.com|linkedin\.com|tiktok\.com|opensea\.io|niftygateway.com)/`)
	// description = socialMediaRegex.ReplaceAllStringFunc(description, func(match string) string {
	...
	// learnRegex := regexp.MustCompile(`https://([^"]+)`)
	// // learnRegex := regexp.MustCompile(`https://www\.coingecko\.com/([^"]+)|https://www\.coindesk\.com/([^"]+)|https://www\.luminex\.io/([^"]+)|https://www\.decrypt\.co/([^"]+)|https://www\.mirror\.xyz/([^"]+)|https://www\.ambr\.live/([^"]+)|https://www\.blox\.pro/([^"]+)|https://www\.vivid\.limited/([^"]+)`)
	...
	// remove all foreign link from coingecko
	...
	description = foreignLinkRegex.ReplaceAllStringFunc(description, func(match string) string {
		...
		for _, socialMedia := range socialMediaDomains {
			if strings.Contains(lowerMatch, socialMedia) {
				return match // keep social media links
			}
		}
		return ""
	})
