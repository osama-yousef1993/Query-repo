...
// Gets the occurence_time and id for all assets where it was the first entry
func (bq *BQStore) GetFundamentalsFirstOccuranceTime(ctx0 context.Context, uuid string) (map[string]PGFundamentalsResult, error) {
	...
	var fundamentalsResults = make(map[string]PGFundamentalsResult)
	for {
		var fundamentalsResult PGFundamentalsResult
		err := it.Next(&fundamentalsResult)
		if err == iterator.Done {
		...
		fundamentalsResults[fundamentalsResult.Symbol] = fundamentalsResult
