...
// Rowy table for custom_category fields
var CustomCategoryTable = fmt.Sprintf("%s%s", os.Getenv("ROWY_PREFIX"), "custom_category")
...
type CustomCategoryRequest struct {
	...
	IsHighlighted   bool        `json:"is_highlighted,omitempty" firestore:"isHighlighted"`    // It presents if this custom category is highlighted
}

// Will use ths struct to map the data we will inserted to PG
type CustomCategory struct {
	CategoryName   string                     `json:"categoryName,omitempty" postgresql:"category_name"`     // It presents the custom category name
	CategorySlug   string                     `json:"categorySlug,omitempty" postgresql:"category_slug"`     // It presents the custom category slug
	CategoryFields []byte                     `json:"categoryFields,omitempty" postgresql:"category_fields"` // It presents the markets we will added to custom category
	Markets        []Fundamentals             `json:"markets,omitempty" postgresql:"markets"`                // It presents the markets we will added to custom category
	NFTs           []NFTsTable                `json:"nfts,omitempty" postgresql:"nfts"`                      // It presents the NFTs we will added to custom category
	Categories     []CategoryFundamentalTable `json:"categories,omitempty" postgresql:"categories"`          // It presents the markets we will added to custom category
	CategoryType   string                     `json:"categoryType,omitempty" postgresql:"category_type"`     // It presents the custom category type ex(FT, NFT and CATEGORY)
	CategoryPath   string                     `json:"categoryPath,omitempty" postgresql:"category_path"`     // It presents the custom category path ex(categories or highlights)
	IsHighlighted  bool                       `json:"is_highlighted" postgresql:"is_highlighted"`            // It presents if this custom category is highlighted
	InActive       bool                       `json:"inactive" postgresql:"inactive"`                        // It presents if this custom category active to be build
	LastUpdated    time.Time                  `json:"lastUpdated,omitempty" postgresql:"last_updated"`       // It presents last time the row updated
}
