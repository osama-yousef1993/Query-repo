...
func GetNFTPricesTest(w http.ResponseWriter, r *http.Request) {
	setResponseHeaders(w, 60)
	labels := make(map[string]string)
	...
	result, err := store.PGGetNFTPrices(ctx)
	if err != nil {
		log.ErrorL(labels, "%s", err)
		span.SetStatus(codes.Error, err.Error())
		w.WriteHeader(http.StatusNotFound)
...
// NFT Dictionary. Unused at the moment.
...
func SearchNFTTerm(ctx context.Context, searchTerm string, dictionaryCategory DictionaryCategory, paginate Paginate) ([]byte, error) {
	...
	exactMatchNFTs, isExact := dictionary.nfts[searchTerm] //Assets that directly match the search term.
	exactMatchNFTs = RemoveDuplicateInactiveNFTs(ctx0, exactMatchNFTs)
	if isExact {
		nfts = append(exactMatchNFTs, nfts...) // append the direct match assets to the assets array in the front.
	}
	nfts = RemoveDuplicateInactiveNFTs(ctx0, nfts)
...
func RemoveDuplicateInactiveNFTs(ctx0 context.Context, nfts []NFTPrices) []NFTPrices {
	_, span := tracer.Start(ctx0, "RemoveDuplicateInactiveAssets")
	defer span.End()
...
func SearchTermByChains(ctx context.Context, searchTerm string, dictionaryCategory DictionaryCategory, paginate Paginate) ([]byte, error) {
	...
	*/
	...
	exactMatchAssets, isExact := nfts[searchTerm] //Assets that directly match the search term.
	exactMatchAssets = RemoveDuplicateInactiveNFTs(ctx0, exactMatchAssets)
	if isExact {
		nftsResult = append(exactMatchAssets, nftsResult...) // append the direct match assets to the assets array in the front.
	}
	nftsResult = RemoveDuplicateInactiveNFTs(ctx0, nftsResult)
