...
func (m *Microservices) isMemberInfoRequestAuthorized(r *http.Request) dto.MagicDIDValidationResults {
	...
	if !results.IsDIDValid {
		results, err := m.tokenManagerService.ValidateTokenFormat(r.Context(), did)
		if err != nil {
			log.Error(fmt.Sprintf("%s", err))
			return results
		}
...
func (m *Microservices) ValidateToken(w http.ResponseWriter, r *http.Request) {
	...
	if token == "" {
		results.IsAuthorized = false
	}
	results, _ = m.tokenManagerService.ValidateTokenFormat(r.Context(), token)
