...
type TokenManagerService interface {
	...
	ValidateTokenFormat(ctx context.Context, tokenFormate string) (dto.MagicDIDValidationResults, error)
	GenerateJWTTest(ctx context.Context, details dto.JWT) (string, error)
}
...
func (t *tokenManagerService) GenerateJWT(ctx context.Context, details dto.JWT) (string, error) {
	...
	switch details.ISS {
	case "fbs_web3":
		claims, privateKey, err = t.BuildFbsWeb3Token(ctx, details)
	case "FDA":
		claims, privateKey, err = t.BuildFDAClgnToken(ctx, details)
	}
	if err != nil {
...
func (t *tokenManagerService) BuildFbsWeb3Token(ctx context.Context, details dto.JWT) (*jwt.MapClaims, *rsa.PrivateKey, error) {
	span, labels := common.GenerateSpan("tokenManagerService.GenerateJWT", ctx)
	defer span.End()
	span.AddEvent(fmt.Sprintf("Starting %s", "tokenManagerService.GenerateJWT"))
...
func (t *tokenManagerService) BuildFDAClgnToken(ctx context.Context, details dto.JWT) (*jwt.MapClaims, *rsa.PrivateKey, error) {
	span, labels := common.GenerateSpan("tokenManagerService.GenerateJWT", ctx)
	defer span.End()
	span.AddEvent(fmt.Sprintf("Starting %s", "tokenManagerService.GenerateJWT"))
	...
	privateKey, err := jwt.ParseRSAPrivateKeyFromPEM([]byte(t.signaturePrivateKey))
	if err != nil {
		log.EndTime("tokenManagerService.ParseJWT", startTime, nil)
		span.SetStatus(codes.Error, err.Error())
		return nil, nil, err
...
// todo create reverse function to check the private key not the public key
...
func (t *tokenManagerService) ParseJWT(ctx context.Context, tokenString string) (*dto.JWT, error) {
	...
	publicKey, err = jwt.ParseRSAPublicKeyFromPEM([]byte(t.signaturePublicKey))
	if err != nil {
		log.Alert(err.Error())
...
func (t *tokenManagerService) ParseJWTPrivateKey(ctx context.Context, tokenString string) (*dto.JWT, error) {
	// Read the private key from the file
	span, labels := common.GenerateSpan("tokenManagerService.ParseJWT", ctx)
	defer span.End()
	span.AddEvent(fmt.Sprintf("Starting %s", "tokenManagerService.ParseJWT"))
	...
	var (
		unparsedData dto.JWT         // object we are parsing our object to
		err          error           // error to return
		privateKey   *rsa.PrivateKey //rsa public key generated by  jwt.ParseRSAPublicKeyFromPEM
		token        *jwt.Token      // token generated by jwt.Parse
	)
...
func (t *tokenManagerService) JWTVerification(ctx context.Context, tokenString string) (*dto.JWT, error) {
	// Read the private key from the file
	span, labels := common.GenerateSpan("tokenManagerService.ParseJWT", ctx)
	defer span.End()
	span.AddEvent(fmt.Sprintf("Starting %s", "tokenManagerService.ParseJWT"))
...
func (t *tokenManagerService) ValidateTokenFormat(ctx context.Context, tokenFormate string) (dto.MagicDIDValidationResults, error) {
	...
	// todo this token create using public key
	// todo create new one using private key
	jwtToken, err := t.JWTVerification(ctx, tokenFormate)
	if err != nil {
		return results, nil
	}
	results.Issuer = jwtToken.ISS
	results.WalletAddress = jwtToken.AUD
	...
	if jwtToken.SUB == "signature_verification" && jwtToken.ISS == "community_page" && jwtToken.AUD == "FDA" {
		...
		if verify {
			...
			token, _ := t.GenerateJWT(ctx, dtoJWT)
			results.JWT = token
	...
	if jwtToken.SUB == "fda_clgn" && jwtToken.ISS == "FDA" && expirationTime.Before(time.Now().AddDate(0, 0, 7)) {
		results.IsAuthorized = true
		log.EndTimeL(labels, "tokenManagerService.ValidateTokenFormat", startTime, nil)
		span.SetStatus(codes.Ok, "success")
		return results, nil
...
func (t *tokenManagerService) VerifySignature(ctx context.Context, address string, signature string, message string) bool {
	...
	if (parsedTime.Equal(time.Now()) || parsedTime.After(time.Now())) && parsedTime.Before(time.Now().Add(10*time.Minute)) {
		sig := hexutil.MustDecode(signature)

		messageBytes := []byte(message)
		messageBytes = accounts.TextHash(messageBytes)
		sig[crypto.RecoveryIDOffset] -= 27 // Transform yellow paper V from 27/28 to 0/1

		recovered, err := crypto.SigToPub(messageBytes, sig)
		if err != nil {
			return false
		}
		recoveredAddr := crypto.PubkeyToAddress(*recovered)
		add := recoveredAddr.Hex()
		log.EndTimeL(labels, "tokenManagerService.ParseJWT", startTime, nil)
		span.SetStatus(codes.Ok, "success")
		return address == strings.ToLower(add)
...
// Generates a ne jwt token string based of of a dto.jwt.
func (t *tokenManagerService) GenerateJWTTest(ctx context.Context, details dto.JWT) (string, error) {
	span, labels := common.GenerateSpan("tokenManagerService.GenerateJWT", ctx)
	defer span.End()
	span.AddEvent(fmt.Sprintf("Starting %s", "tokenManagerService.GenerateJWT"))
	...
	expirationTime := time.Now().AddDate(0, 0, 7)
	// curretnTime := time.Now().UTC().Add(7 * time.Minute)
	// create the payload mapping
	claims = jwt.MapClaims{
		"sub": "fda_clgn",
		"iss": "FDA",
