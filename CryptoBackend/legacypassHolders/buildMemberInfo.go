...
func (m *Microservices) GetLegacyPassHolderProfiles(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json; charset=utf-8")
	cacheValue := fmt.Sprintf("max-age=%v, public", 300)
	...
	memberInfo, err := m.memberReportService.GetLegacyPassHolderProfiles(r.Context())
	...
	res, err := json.Marshal(memberInfo)
	if err != nil {
		log.ErrorL(labels, "%s", err)
		span.SetStatus(codes.Error, err.Error())
		w.WriteHeader(http.StatusInternalServerError)
