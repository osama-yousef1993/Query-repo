...
const (
	...
	CacheKey      = "alert_rules_cache"
)

// We will use this to map the Transaction data from PubSub
type EthereumTransactionRow struct {
	...
	ValueLossless            string       `json:"value_lossless"`
...
type WhaleTrackerAlertRules struct {
	Entity          []string        `json:"entity" firestore:"entity"`                   // WhaleTracker Entity: ex ftx,binance etc. If this entity is receiving or sending the amount within our threshold we will send an alert
	...
	Token           []string        `json:"token" firestore:"token"`                     // token Ex: BUSD,USDT,BTC,ETH
	Chain           string          `json:"chain" firestore:"chain"`                     // chain Ex: arbitrum,ethereum
...
// PubSubMessage represents the payload of a Pub/Sub event.
