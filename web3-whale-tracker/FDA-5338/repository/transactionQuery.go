...
type TransactionsQuery interface {
	...
	GetAlertRules(ctx context.Context) ([]datastruct.WhaleTrackerAlertRules, error)
}
...
func (t *transactionsQuery) GetTransactions(ctx context.Context, wallets []string) []datastruct.BQTransaction {
	...
	messageHandler := func(ctx context.Context, msg *pubsub.Message) {
		...
		for _, rule := range rules {
			// if rule.Entity[0].Name == "" {
			// 	fmt.Printf("%s%s", transaction.FromAddress, transaction.TransactionHash)
			// } else if rule.Entity[0].Name == transaction.FromAddress {
			// 	fmt.Printf("%s%s", transaction.FromAddress, transaction.TransactionHash)
			// }
			min, err := strconv.ParseFloat(strings.ReplaceAll(rule.MinUSDThreshold, ",", ""), 64)
			if err != nil {
				log.Error(" Could Not parse min value setting defaulting the value to 1.0 %s", err)
				min = 1.0
			}
			max, err := strconv.ParseFloat(strings.ReplaceAll(rule.MaxUSDThreshold, ",", ""), 64)
			if err != nil {
				log.Error(" Could Not parse min value setting defaulting the value to 1.0 %s", err)
				max = 10000000.0
			}
...
func (t *transactionsQuery) GetAlertRules(ctx context.Context) ([]datastruct.WhaleTrackerAlertRules, error) {
	span, labels := common.GenerateSpan("GetAlertRules", ctx)
	defer span.End()

	span.AddEvent(fmt.Sprintf("Starting %s", "GetAlertRules"))
	startTime := log.StartTimeL(labels, fmt.Sprintf("Starting %s", "GetAlertRules"))
	fs, err := fsUtils.GetFirestoreClient()
	if err != nil {
		log.ErrorL(labels, "GetAlertRules Error %s", err)
		span.SetStatus(codes.Error, err.Error())
		return nil, err
	}
	...
	log.EndTimeL(labels, "GetAlertRules", startTime, nil)
	span.SetStatus(codes.Ok, "GetAlertRules")
	return rules, nil
}
