...
var (
	...
	slackAPITOPKEN = os.Getenv("SLACK_API_TOKEN")
	api            = slack.New(slackAPITOPKEN)
	color          = "64a96b"
)
...
func SendSlack(record datastruct.Transaction) {
	var (
		transactionURL    = ""
		blockchainScanner = ""
	)
	...
	tableData := [][]string{
		...
		{formatWalletAddress(record.FromAddress), formatWalletAddress(record.ToAddress), fmt.Sprintf("%.2f", record.Value), formatWalletAddress(record.FromAddress), formatWalletAddress(record.ToAddress)},
	}
