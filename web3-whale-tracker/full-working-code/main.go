...
func main() {
	...
	r.HandleFunc("/consume-pubsub-messages", microservices.BuildWhaleTrackerData).Methods(http.MethodPost, http.MethodOptions)
	r.HandleFunc("/wallets-entity", microservices.GetWalletsEntities).Methods(http.MethodGet, http.MethodOptions)
	r.HandleFunc("/receive-transactions", microservices.ReceiveTransactions).Methods(http.MethodPost, http.MethodOptions)
