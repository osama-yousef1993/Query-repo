WITH
  ...
  SELECT
    current_table.id,
    100 * ((current_table.volumeUSD_1d - previous_table.volumeUSD_1d) / NULLIF(previous_table.volumeUSD_1d, 0)) AS pct_change_volume_usd_1d,
    100 * ((current_table.volumeUSD_7d - previous_table.volumeUSD_7d) / NULLIF(previous_table.volumeUSD_7d, 0)) AS pct_change_volume_usd_7d,
    100 * ((current_table.volumeUSD_30d - previous_table.volumeUSD_30d) / NULLIF(previous_table.volumeUSD_30d, 0)) AS pct_change_volume_usd_30d,
    100 * ((current_table.volumeUSD_90d - previous_table.volumeUSD_90d) / NULLIF(previous_table.volumeUSD_90d, 0)) AS pct_change_volume_usd_90d,
    100 * ((current_table.volumeUSD_ytd - previous_table.volumeUSD_ytd) / NULLIF(previous_table.volumeUSD_ytd, 0)) AS pct_change_volume_usd_ytd,
    100 * ((current_table.volumeNative_1d - previous_table.volumeNative_1d) / NULLIF(previous_table.volumeNative_1d, 0)) AS pct_change_volume_native_1d,
    100 * ((current_table.volumeNative_7d - previous_table.volumeNative_7d) / NULLIF(previous_table.volumeNative_7d, 0)) AS pct_change_volume_native_7d,
    100 * ((current_table.volumeNative_30d - previous_table.volumeNative_30d) / NULLIF(previous_table.volumeNative_30d, 0)) AS pct_change_volume_native_30d,
    100 * ((current_table.volumeNative_90d - previous_table.volumeNative_90d) / NULLIF(previous_table.volumeNative_90d, 0)) AS pct_change_volume_native_90d,
    100 * ((current_table.volumeNative_ytd - previous_table.volumeNative_ytd) / NULLIF(previous_table.volumeNative_ytd, 0)) AS pct_change_volume_native_ytd
  FROM (
    SELECT
      *
    FROM
...
SELECT
  ...
  IFNULL(pct_change_volume_usd_1d,0) pct_change_volume_usd_1d,
  IFNULL(pct_change_volume_usd_7d,0) pct_change_volume_usd_7d,
  IFNULL(pct_change_volume_usd_30d,0) pct_change_volume_usd_30d,
  IFNULL(pct_change_volume_usd_90d,0) pct_change_volume_usd_90d,
  IFNULL(pct_change_volume_usd_ytd,0) pct_change_volume_usd_ytd,
  IFNULL(pct_change_volume_native_1d,0) pct_change_volume_native_1d,
  IFNULL(pct_change_volume_native_7d,0) pct_change_volume_native_7d,
  IFNULL( pct_change_volume_native_30d,0) pct_change_volume_native_30d,
  IFNULL(pct_change_volume_native_90d,0) pct_change_volume_native_90d,
  IFNULL(pct_change_volume_native_ytd, 0) pct_change_volume_native_ytd
FROM
  YTD
LEFT JOIN
  ninetyDay
