...
func init() {
	var err error
	microServices, err = app.NewMicroservices(coinListService)
	if err != nil {
...
func main() {
	...
	r.Handle("/consume-coin-paprika-exchanges-list", oidcAuthMiddleware.Middleware(http.HandlerFunc(microServices.BuildCoinList))).Methods(http.MethodPost)
	r.Handle("/consume-coin-paprika-exchanges-markets", oidcAuthMiddleware.Middleware(http.HandlerFunc(microServices.BuildCoinMetaDataList))).Methods(http.MethodPost)
